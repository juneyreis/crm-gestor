...
import { Search, Filter, ArrowUpDown, Shield, CheckCircle, XCircle, Info } from 'lucide-react';

export default function ConcorrentesReport() {
    ...
    const loadData = async () => {
        ...
        setLoading(true);
        try {
            const result = await concorrentesService.listarConcorrentes(user.id);
            setData(result || []);
        } catch (error) {
            console.error('Erro ao carregar dados do relatório de concorrentes:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleExportPDF = () => {
        generatePDF('report-content', 'relatorio-concorrentes.pdf');
    };
    ...
    const totalNaoPlotados = totalConcorrentes - totalPlotados;
    ...
    const filters = (
        <>
            <div className="relative flex-grow">
                ...
                <input
                    type="text"
                    placeholder="Filtrar por descrição ou observação..."
                    value={searchTerm}
            ...
            >
                ...
                <option value="yes">Ativos no Mapa</option>
    ...
    return (
        <ReportLayout
            ...
            subtitle="Análise detalhada de sistemas e empresas concorrentes"
            filters={filters}
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-100 dark:border-slate-700/50 flex items-center justify-between">
                    <div>
                        <p className="text-xs text-slate-500 uppercase font-bold tracking-wider mb-1">Total Concorrentes</p>
                        <p className="text-xl font-black text-slate-900 dark:text-white">{totalConcorrentes}</p>
                    </div>
                    <div className="p-1.5 bg-white dark:bg-slate-700 rounded-full shadow-sm">
                        <Shield className="h-4 w-4 text-slate-400" />
                    </div>
                </div>
                <div className="bg-blue-50 dark:bg-blue-900/10 p-3 rounded-lg border border-blue-100 dark:border-blue-900/20 flex items-center justify-between">
                    <div>
                        <p className="text-xs text-blue-600 dark:text-blue-400 uppercase font-bold tracking-wider mb-1">Ativos (Mapa)</p>
                        <p className="text-xl font-black text-blue-700 dark:text-blue-400">{totalPlotados}</p>
                    </div>
                    <div className="p-1.5 bg-white dark:bg-blue-900/20 rounded-full shadow-sm">
                        ...
                        {filteredData.length > 0 ? (
                            filteredData.map((item) => (
                                ...
                                >
                                    ...
                                    <td className="py-2 px-4 text-sm text-slate-500 dark:text-slate-400">
                            ...
                            <tr>
                                <td colSpan="4" className="py-12 text-center text-slate-400 italic bg-slate-50/30 dark:bg-slate-800/20">
            ...
            <div className="md:hidden space-y-4">
                {filteredData.map((item) => (
                    <div key={item.id} className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-100 dark:border-slate-700 shadow-sm animate-fade-in-up">
                        ...
                        {item.observacoes && (
                            <div className="mt-3 flex gap-2 items-start p-2 rounded bg-slate-50 dark:bg-slate-900/50">
                                <Info className="h-3.5 w-3.5 text-slate-400 mt-0.5" />
                                <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2">{item.observacoes}</p>
                            </div>
                        )}
                    </div>
                ))}
