...
export default function ComboboxClientes({
    options = [],
    value,
    onChange,
    name,
    ...
    const filteredOptions = useMemo(() => {
        ...
        return options.filter(opt => {
            const label = labelPath(opt);
            return label?.toLowerCase().includes(term);
        });
    }, [options, searchTerm, labelPath]);

    const selectedOption = useMemo(() => {
        return options.find(opt => String(opt.id || opt.value || opt) === String(value));
    }, [options, value]);

    const handleSelect = (opt) => {
        const val = opt.id || opt.value || opt;
                        ...
                        {filteredOptions.length > 0 ? (
                            filteredOptions.map((opt, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        <span className="truncate">{labelPath(opt)}</span>
                            ...
                            <div className="p-4 text-center">
                                <p className="text-sm text-gray-500 italic">Nenhuma opção encontrada</p>
                            </div>
                        )}
                    </div>
                </div>
