...
import ConcorrenteMobileCard from './ConcorrenteMobileCard';

export default function ConcorrenteTable({
    concorrentes,
    loading,
    error,
    onEdit,
    ...
    if (loading && concorrentes.length === 0) {
        return <div className="p-12 text-center text-gray-500 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">Carregando concorrentes...</div>;
    }
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                        {concorrentes.map((concorrente) => (
                            <tr key={concorrente.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors group">
                                ...
                                <td className="px-6 py-4 text-center">
                                    <button
                                        ...
                                        className={`inline-flex items-center justify-center p-1.5 rounded-full transition-all border ${concorrente.plotar
